...
[15-Oct-09] Added interpEvent() to make deadlock report more readily
	understandable. V1.1f (BG)
[16-May-11] Fixed #31: Try to keep Pilot messages separate from MPI msgs to
	...
	Added a "key" explaining notation on deadlock traceback. V2.0 (BG)
[17-Sep-16] Fixed blocked message, source:line was in wrong place, wasn't
        printing format arg.
*******************************************************************************/
...
static int EQmaxlen = 0;	/*!< keeps track of max growth */
...
static char eventCodes[] = {
	// CALLS events
	"CWri" "CRea" "CSel" "CHas" "CTry" "CBro" "CGat" "CSca" "CRdu"
	// PILOT events
	"PFIN" };
...
{
////// suppress CTry and CHas events, since they don't affect dependencies
if ( strstr(evt,"Try") || strstr(evt,"Has") ) return;
////// this is a crude temporary way to do it
...
/*!
********************************************************************************
Table of function codes vs. full name & flags for channel/bundle and format.
This covers the same cases as recognized eventCodes, except for PFIN.
*******************************************************************************/
static struct {
    char code[4];	// 3-char code + NUL
    char *fname;	// function name
    char chan;		// 'C' takes channel, or 'B' bundle ID
    char fmt;		// 'F' takes format string
} funcCalls[] = {
	{ "Wri", "PI_Write", 'C', 'F' },
	{ "Rea", "PI_Read", 'C', 'F' },
	{ "Sel", "PI_Select", 'B', '-' },
	{ "Has", "PI_ChannelHasData", 'C', '-' },
	{ "Try", "PI_TrySelect", 'B', '-' },
	{ "Bro", "PI_Broadcast", 'B', 'F' },
	{ "Gat", "PI_Gather", 'B', 'F' },
	{ "Sca", "PI_Scatter", 'B', 'F' },
	{ "Rdu", "PI_Reduce", 'B', 'F' },
	{ "ZZZ", "sentinel", '-', '-'} };	// <-- must end with Z!

/*!
********************************************************************************
Interpret event text for user. Returns pointer to static buffer.
*******************************************************************************/
static char *interpEvent( const char *event )
{
    // add expansion factor for function name to max len
    static char buff[PI_MAX_LOGLEN+20];

    char *strtokLast = NULL;		// used by strtok_r to continue parsing

    char *text = strdup( event );	// make copy that strtok can modify
    PI_OLP_ASSERT( text, PI_MALLOC_ERROR );

    // we needn't check assertions on event text, since it's been parsed before

    strtok_r( text, PI_LOGSEP, &strtokLast );	// skip over event type

    // next token should be process ID
    const char *tok = strtok_r( NULL, PI_LOGSEP, &strtokLast );
    int proc = atoi( tok );

    // next token should be 3-char function code
    const char *func = strtok_r( NULL, PI_LOGSEP, &strtokLast );

    // next token should be channel or bundle ID
    tok = strtok_r( NULL, PI_LOGSEP, &strtokLast );
    int chanbund = atoi( tok );

    // next token should be source file and line no.
    const char *source = strtok_r( NULL, PI_LOGSEP, &strtokLast );

    // remainder, if any, will be format string and possible value
    tok = strtok_r( NULL, PI_LOGSEP, &strtokLast );

    // find the matching code and fill in the interpretation
    int i;
    for ( i=0; funcCalls[i].code[0]!='Z'; i++ ) {
	if ( strncmp( func, funcCalls[i].code, 3 ) != 0 ) continue;

	const char *cbID, *from, *to;	// all point to existing strings
	const char *sep;		// separator for from/to

	if ( funcCalls[i].chan == 'C' ) {
	    cbID = olpe->channels[chanbund-1]->name;
	    from = olpe->processes[olpe->channels[chanbund-1]->producer].name;
	    to = olpe->processes[olpe->channels[chanbund-1]->consumer].name;
	    sep = ">";
	} else {
	    cbID = olpe->bundles[chanbund-1]->name;
	    from = olpe->processes[ 
		    olpe->bundles[chanbund-1]->channels[0]->producer ].name;
	    to = olpe->processes[ 
		    olpe->bundles[chanbund-1]->channels[0]->consumer ].name;

	    if ( olpe->bundles[chanbund-1]->narrow_end == FROM )
		sep = ">*";
	    else
		sep = "*>";
	}

	char *openFmt, *closeFmt;

	if ( funcCalls[i].fmt == 'F' ) {	// format is in tok
	    openFmt = ", \"";
	    closeFmt = "\"";
	} else {
	    tok = openFmt = closeFmt = "";
	}

	// interpretation:
	//	Process 'name'(arg) @ source:line called func(chan/bund:from>to[,fmt])
	snprintf( buff, sizeof(buff)-1,
		"Process %s(%d) @ %s called %s(%s:%s%s%s%s%s%s)",
		olpe->processes[proc].name,
		olpe->processes[proc].argument,
                source,
		funcCalls[i].fname,
		cbID, from, sep, to,
		openFmt, tok, closeFmt );

	free( text );
	return buff;
    }

    // should not happen: failed to lookup code, so return raw event
    strncpy( buff, event, sizeof(buff) );
...
/*!
********************************************************************************
Abort upon discovering deadlock, printing triggering event and reason.
*******************************************************************************/
static void abortDL( int procID, const char *event, const char *reason )
{
    fprintf( stderr,
        "\n"
	PI_BORDER "*** BLOCKED: %s\n"
	PI_BORDER "*** Deadlock detected in Pilot process %s(%d)!!!\n"
        PI_BORDER "***   %s\n"
        PI_BORDER "*** See above for BLOCKED process status.\n"
        PI_BORDER "***   Other unreported processes may be blocked.\n"
	PI_BORDER "*** Key to notation:\n"
	PI_BORDER "***   P#, C#, B# = process, channel, bundle; # in order created\n"
	PI_BORDER "***   @P# = process at common end of bundle\n"
	PI_BORDER "***   (i) = index argument of process\n"
	PI_BORDER "***   s:n = source file:line number\n"
        PI_BORDER "***   (x:y>z, \"f\"):\n"
	PI_BORDER "***     x = channel or bundle argument, direction from y to z\n"
	PI_BORDER "***     * = first process at wide end of bundle\n"
	PI_BORDER "***     f = format argument\n",
                interpEvent(event),
...
printf( "$DL$ +++ Select by P%d, P%d is running\n", p, r );
	...
	signed char thisDep = DEPENDS(p,r);
	...
	DEPENDS(p,r) = 0;		// zero this dep. to prevent inf. rec.
        if ( isCycle( r, q, print ) ) {
            if (print) fprintf( stderr, PI_BORDER "*** BLOCKED: %s\n",
                      interpEvent(process[p].lastEvent) );
...
printf( "select complete!\n" );
    ...
    */
    ...
    // if q is doing the select, p will not fulfill it, so...
    if ( DEPENDS(q,p) == -2 ) {
	...
	/* Strictly speaking, we should clear chanproc[ch] where ch is the
	   ...
	   If p was the last process that could have fulfilled the select,
	   when we decrement q's state count it will drop to RUN.
	   Then q is in trouble because p's action means there's no one left to
	   fulfill its earlier select.  Print what q was doing (select).
	*/
	if ( --(process[q].state) == RUN ) {
	    fprintf( stderr, PI_BORDER "*** BLOCKED: %s\n", interpEvent(process[q].lastEvent) );
	    abortDL( pevt->proc, pevt->saveEvent,
...
printf( "$DL$ * handling %d '%4s'\n", ev->proc, ev->ecode );
	...
	case 7: // PI_Scatter
	    ...
	    // make write dependencies p->{consumers of Broadcaster/Scatterer bundle}
	...
	case 8: // PI_Reduce
	    ...
	    // make read dependencies p->{producers of Gatherer/Reducer bundle}
	    for ( i = 0; i<bundsize; i++ )
		makeDepend( ev, bundchan[i]->producer, bundchan[i]->chan_id, -1 );
	    break;

	case 9: // process exited
...
printf( "$DL$ Recd & queued: [%s]\n", event );
    ...
    /* Repeatedly scan event queue, because handling any event could unblock
       ...
       now empty.  There can be multiple queued events from the same blocked
       process, because PI_Write may return as soon as data leaves user's
       buffer, allowing caller to go on and make other Pilot calls.  This is
       why we start a fresh scan anytime an event was handled.
    */
    int foundWork;
    EQevent *ev;
...
void PI_DetectDL_end_()
{
    ...
    free( process );
    free( chanproc );
printf( "$DL$ signing off, max queue len = %d\n", EQmaxlen );
}
