...
RUNNER=mpiexec	# how to run MPI programs (e.g., mpiexec)
...
run_test() {
    ...
    printf "  $1... "
    tmp=$($RUNNER -np $NPROCS "deadlock/$1.case" -pisvc=d 2>&1 > /dev/null)
...
run_test "scatter_deadly_embrace"	"$REASON_DE"
run_test "test_dead_wait_reduce"	"$REASON_OX" "$REASON_XH"
